...
s1 = [(h_a/2 + t_sp - C_airfoil)*np.ones((s_reso))                                              , s_spar[::-1] - h_a/2                  , s_spar]
...
s4 = [(h_a/2 + t_sp - C_airfoil)*np.ones((s_reso))                                              , s_spar - h_a/2                        , s_spar]
...
for stif in botcirc_stify:
    y2 += McCauley0(-h_a/2*np.sin(s_qcirc[::-1]/(h_a/2)), stif)*stif
for stif in bottail_stify:
    y3 += (McCauley0(-s_tail*m.sin(alpha_tail), stif) -1)*(-1)*stif
for stif in toptail_stify:
    y5 += (McCauley0(s_tail[::-1]*m.sin(alpha_tail), stif) -1)*(-1)*stif
